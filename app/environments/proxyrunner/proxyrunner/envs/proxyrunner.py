...
class ProxyEnv(gym.Env):
    ...
    def __init__(self, verbose = False, manual = False):
        ...
        self.observation_space = gym.spaces.Box(-1, 1, (self.num_squares + 2*self.grid_length + 2 + self.action_space.n, ))
        self.verbose = verbose
        self.opp_model_pool = []

        opp_dir = os.path.join(config.MODELDIR, "proxychaser")
    ...
    def step(self, action):
        ...
        while self.current_player_num != MAIN_PLAYER_ID and (not self.done):
            ...
            opponent_obs = self.get_chaser_obs()
            ...
            r, done = self.game_step(action_chosen)
    ...
    def reset(self):
        ...
        self.players = [Player('Runner', Token('X', 1)), Player('Chaser', Token('O', -1))]
