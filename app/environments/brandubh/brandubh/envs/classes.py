...
################## Internal methods ##################
    ...
    def _isLegalMove(self,pieceno,x2,y2) -> int:
      try:
    ...
    def _moveByPieceNo(self,pieceno,x2,y2):
      ...
      piece=self.pieces[pieceno]
      foo = "piece" if (piece[2] != 2) else "KING"
      logger.debug(f"-> Moved {foo} from ({piece[0]},{piece[1]}) to ({x2},{y2})")
      piece[0]=x2
      piece[1]=y2
      caps = self._getCaptures(pieceno,x2,y2)
      #print("Captures = ",caps)
      for c in caps:
          logger.debug(f"  --- Piece Captured at ({c[0]},{c[1]}) ---")
          c[0]=-99
