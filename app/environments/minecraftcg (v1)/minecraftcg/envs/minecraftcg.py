...
class MinecraftCGEnv(gym.Env):
    ...
    def __init__(self, verbose = False, manual = False):
        ...
        self.max_score = 25 # depends on number of players: 2=24,3=20,4=16, must give room to go over
        ...
        #    - Plus the one recipe left out of the game?
        + (4 + 2 * self.n_players + self.n_players + 1) * self.total_craft_cards
    ...
    def observation(self):
        ...
        obs2 = np.zeros(([(4 + 2 * self.n_players + self.n_players + 1),self.total_craft_cards]))
        ...
        for i in range(self.n_players):
            ...
            # Reserve
            if p.reserve != None:
                obs2[-self.n_players - 1 + i][p.reserve.id] = 1
        ...
        for i in range(self.n_players):
            p = self.players[player_num]
            score_obs[i] = p.score / self.max_score
            # print(len(ret) + i)
            # print(score_obs[i])
            player_num = (player_num + 1) % self.n_players
