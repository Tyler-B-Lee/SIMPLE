...
class MinecraftCGEnv(gym.Env):
    ...
    def __init__(self, verbose = False, manual = False):
        ...
        self.craft_contents = []
        self.tool_list = ['hoe','shovel','sword','axe','pickaxe']
        for i, material in enumerate(['wood','stone','iron','gold','diamond']):
            for j, tool in enumerate(self.tool_list):
        ...
        self.obs1_size = (
            5 * (self.resource_card_types) # 5 resource piles
            + self.resource_card_types # discard pile
            # player positions (does not count Creepers or TNT)
            + (self.total_resource_cards - 10) * self.n_players
        )

        self.observation_space = gym.spaces.Box(0, 1, (
        # which cards are the top ones for the Draw Piles?
        # Resource cards (we don't need to tell it card ID's, just the amount of that card):
        self.obs1_size

        # Crafting Cards:
        # - 4 Crafting Piles times # of different Craft/Tool Cards + player reserves
        #    - Plus the crafted tool positions for each player (+2 per player, total tools / number usable)
    ...
    def obs_helper(self, lst: list, lim: int):
        """Helps create a portion of the observation with the new name system (vs old ID system).
        'lst' is a list of card objects that we want to count from, and 'lim' is an integer that is
        the last card type we want to look for; 18 is for materials only, while 20 includes TNT and creepers.
        
        Returns a numpy array of 1's and 0's in order for the number counted, for 'lim' types of cards."""
        out = np.zeros(0)
        counter = {}
        for card in lst:
            n = card.name
            if n in counter:
                ...
                counter[n] = 1
        for i in range(lim):
            foo = np.zeros((self.resource_contents[i]['count'],))
            target_name = self.resource_contents[i]['info']['name']
            if target_name in counter:
                foo[counter[target_name] - 1] = 1
            # logger.debug(f"{lim} Observation for {target_name}: {foo}")
            out = np.append(out, foo)
        return out
        
    @property
    def observation(self):
        # first, locations of Resource Cards
        obs = np.zeros((5, self.resource_card_types))
        obs2 = np.zeros(((4 + 2 * self.n_players),self.total_craft_cards))
        ...
        for i in range(5):
            pile_i = piles[i]
            if pile_i.size() > 0:
                top_card_name = pile_i.peek().name
                obs[i][self.name_action_mapping[top_card_name]] = 1
        obs = obs.flatten()
        ...
        for i in range(self.n_players):
            ...
            # for tool inventory of current player
            obs3 = np.zeros((5,5))
            counter = {'hoe':0,'shovel':0,'sword':0,'axe':0,'pickaxe':0}

            # Resource Cards
            player_resource_list = p.resources.cards
            obs = np.append(obs, self.obs_helper(player_resource_list, 18))
            ...
            for i,tool in enumerate(self.tool_list):
                ...
                if j > 0:
                    obs3[i][j - 1] = 1
            foo = np.append(foo, obs3.flatten())
        ...
        obs2 = np.append(obs2.flatten(), foo)

        # Fraction of total of that card in the discard pile
        foo = np.zeros((self.resource_card_types,))
        counter = {}
        for card in self.certain_discarded:
            ...
            if n in counter:
                ...
                counter[n] = 1
        for n in counter:
            i = self.name_action_mapping[n]
            total = self.resource_contents[i]['count']
            foo[i] = counter[n] / total
        ...
        # what the current game state is (regular turn or TNT picking)
        foo = np.zeros((2,))
