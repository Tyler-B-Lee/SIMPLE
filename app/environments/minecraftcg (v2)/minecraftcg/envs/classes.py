...
class ResourceCard(Card):
    ...
    @property
    def symbol(self):
        return f"(id: {self.id}) {self.material.capitalize()} - {self.amount}"
...
class CraftCard(Card):
    ...
    @property
    def symbol(self):
        return f"(id: {self.id}) {self.material.capitalize()} {self.tool.capitalize()} ({self.points} pts) - {'Used' if self.used else 'Usable'}"
